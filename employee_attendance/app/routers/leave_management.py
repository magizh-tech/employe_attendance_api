...
@router.get("/employees/{employee_id}/leaves/", response_model=list[schemas.LeaveResponse])
def get_leaves(employee_id: int, db: Session = Depends(get_db)):
    return crud.get_employee_leaves(db, employee_id)

@router.put("/leaves/{leave_id}/approve", response_model=schemas.LeaveResponse)
